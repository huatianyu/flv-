      ...
      <div class="content-right">
        <div class="content-right-header">
          ...
          <div class="header-right">
            <img
              @click="splitScreenClick(index, item)"
              v-for="(item, index) in btnList"
              :src="getImgUrl(item.selected ? item.selectedBtnImg : item.btnImg)"
            />
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      btnList: [
        { btnImg: 'one-video.png', selected: false, selectedBtnImg: 'selected-one-video.png' },
        { btnImg: 'four-video.png', selected: false, selectedBtnImg: 'selected-four-video.png' },
        { btnImg: 'nine-video.png', selected: false, selectedBtnImg: 'selected-nine-video.png' }
      ]
    }
  },
  mounted: function() {
  ...
  methods: {
    ...
    onSelect(selectedKeys, e) {
      ...
      if (that.videoList.length <= 1) {
        this.splitScreenStyle = 1
      } else if (that.videoList.length > 1 && that.videoList.length <= 4) {
        this.splitScreenStyle = 2
      } else if (that.videoList.length > 4 && that.videoList.length <= 9) {
        this.splitScreenStyle = 3
      } else {
        that.videoList.pop()
